...
def run():
    ...
    parser.add_argument('--output', type=str, default='Car', help='source')
    ...
    capture = cv2.VideoCapture(0)  # 打开摄像头，0表示默认摄像头，如果有多个摄像头可根据实际情况调整编号

    while True:
        ret, frame = capture.read()  # 读取摄像头的图像帧
        if not ret:
            break

        dict_list = detect_Recognition_plate(detect_model, frame, device, plate_rec_model, opt.img_size, is_color=opt.is_color)
        result_frame = draw_result(frame, dict_list)

        cv2.imshow("License Plate Recognition", result_frame)  # 在窗口中显示识别结果
