...
def run(file_path):
    ...
    parser.add_argument('--image_path', type=str, default=file_path, help='source')  # 图片路径
    parser.add_argument('--img_size', type=int, default=640, help='inference size (pixels)')  # 网络输入图片大小
    parser.add_argument('--output', type=str, default='Car', help='source')  # 图片结果保存的位置
    ...
    if file_extension.lower() in ['.png', '.jpg', '.jpeg', '.bmp']:  # 处理图片
    ...
    elif file_extension.lower() in ['.mp4', '.avi']:  # 处理视频
        ...
        out = cv2.VideoWriter('Car/result.mp4', fourcc, fps, (width, height))  # 写入视频
        frame_count = 0
        fps_all = 0
        ...
        return  cv2.VideoCapture('Car/result.mp4')